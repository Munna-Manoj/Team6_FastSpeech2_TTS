...
class FFN(nn.Module):
    ...
    def forward(self, input_, stop=False):
        # FFN Network
...
class Label(nn.Module):
	"""
	Label injection to both prenet and mel linear parts
	"""
	def __init__(self, input_size, hidden_size, output_size, p=0.5, bias=False):
		...
		self.hidden_size = hidden_size
		self.layer = nn.Sequential(OrderedDict([
		     ('fc1', Linear(self.input_size, self.hidden_size, bias=False)),
		     ('relu1', nn.ReLU()),
		     ('dropout1', nn.Dropout(p)),
		     ('fc2', Linear(self.hidden_size, self.hidden_size, bias=False)),
		     ('relu2', nn.ReLU()),
		     ('dropout2', nn.Dropout(p)),
             ('fc3', Linear(self.hidden_size, self.hidden_size // 2, bias=False)),
		     ('relu3', nn.ReLU()),
		     ('dropout3', nn.Dropout(p)),
		     ('fc4', Linear(self.hidden_size // 2, self.hidden_size // 3, bias=False)),
		     ('relu4', nn.ReLU()),
		     ('dropout4', nn.Dropout(p)),
		     ('fc5', Linear(self.hidden_size // 3, self.output_size, bias=False)),
		     ('relu5', nn.ReLU()),
		     ('dropout5', nn.Dropout(p))		     
		]))
