...
class MelDecoder(nn.Module):
    ...
    def __init__(self, num_hidden):
        ...
        self.label_prenet = Label(1, num_hidden * 2, num_hidden, p=0.2)
        self.decoder_prenet = Prenet(hp.num_mels, num_hidden * 2, num_hidden, p=0.2)
        self.norm = Linear(num_hidden, num_hidden)
        self.label_decoder1 = Label(1, num_hidden, num_hidden, p=0.2)
        self.label_decoder2 = Label(1, num_hidden, num_hidden, p=0.2)

        self.selfattn_layers = clones(Attention(num_hidden), 3)
        self.dotattn_layers = clones(Attention(num_hidden), 3)
        self.ffns = clones(FFN(num_hidden), 3)
        
        self.label_postnet = Label(1, num_hidden, hp.num_mels * hp.outputs_per_step, p=0.2)
    ...
    def forward(self, memory, decoder_input, c_mask, pos, label=0):
        ...
        if label == 0:
        	label1 = t.zeros(decoder_input.size(0), decoder_input.size(1), 1).cuda()
        elif label == 1:
        	label1 = t.ones(decoder_input.size(0), decoder_input.size(1), 1).cuda()
        	
        # Decoder pre-network
        decoder_input = self.decoder_prenet(decoder_input)
		
		# Prenet injection label
        label_pre = self.label_prenet(label1)

        decoder_input = decoder_input + label_pre
        ...
        label3 = t.clone(label1)
        label4 = t.clone(label1)
        
        # Decoder injection label
        label_dec1 = self.label_decoder1(label3)
        label_dec2 = self.label_decoder2(label4)

        for selfattn, dotattn, ffn in zip(self.selfattn_layers, self.dotattn_layers, self.ffns):
            decoder_input = decoder_input + label_dec1
            decoder_input, attn_dec = selfattn(decoder_input, decoder_input, mask=mask, query_mask=m_mask)
            decoder_input = decoder_input + label_dec2
        ...
        # label injection
        label2 = t.clone(label1)
        
        # Postnet injection label
        label_post = self.label_postnet(label2)

        
        # Mel linear projection
        mel_out = self.mel_linear(decoder_input)
        mel_out = mel_out + label_post
...
class Model(nn.Module):
    ...
    def forward(self, characters, mel_input, pos_text, pos_mel, label=0):
